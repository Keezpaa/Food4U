...
import kotlinx.android.synthetic.main.holder_hjemside_karusell_element.view.*
...
class HjemsideViewHolder(itemView : View, callback : ElementClickListener) : AbstractViewHolder(itemView,callback) {

    override fun presentData(data: LocalModel) {
        when(data){
            is OppskriftMain -> {
                itemView.oppskrift_karusell_oppskrift_navn.text = data.oppskrift.tittel
                Glide.with(itemView).load(data.oppskrift.bilde).placeholder(R.mipmap.food4u_app_logo)
                    .into(itemView.oppskrift_karusell_bilde)
            }
        }
    }
}